...
> 这一章不是要你背完整的 RISC-V 手册，而是先建立一条能支撑后续实验的主线：**CPU 执行指令 → 指令读写寄存器和内存 → 特权级限制指令能力 → trap 把控制权交给内核 → 页表决定地址最终落到哪里**。后续 Lab 里遇到的 `a0`、`sepc`、`stval`、`satp`、trapframe，都能放回这条主线理解。

#### 本章先解决什么问题

如果只从 C 代码看 PKE，很多关键动作会像“凭空发生”：函数参数为什么在 `a0` 中？用户程序为什么不能直接配置定时器？执行 `ecall` 后为什么会跳到另一个地址？一个虚拟地址为什么能对应完全不同的物理页？这些都不是 C 语言本身能解释的，而是由指令集、ABI、特权级、trap 和页表共同决定的。

#### 需要的前置知识

- 能阅读基本 C 代码，知道指针表示一个地址；
- 知道函数调用会传入参数并返回结果；
- 知道内存可以按字节寻址，但暂时不要求理解页表；
- 不要求记住 RISC-V 指令和 CSR，本章会在具体事件中逐个引入。

#### 推荐学习路线

1. 先读 1.2，理解“一条指令如何改变寄存器”和“函数调用如何使用寄存器”；
2. 再读 1.3，理解为什么同一颗 CPU 要划分 M/S/U 三种权限；
3. 接着读 1.4，把 trap 看成一次由硬件参与的控制流切换；
4. 然后读 1.5，固定使用“虚拟地址拆分—查 PTE—拼物理地址”的模型；
5. 最后读 1.6，把编译器、链接器、Spike、GDB 放回程序运行全流程；
6. 1.1 用来理解 RISC-V 的设计背景，可以先快速阅读、复习时再回看。

#### 学完后你应该能回答

- `addi a0, a0, 1` 中哪个是目的寄存器，哪个是源操作数？
- `a0` 为什么既可能是第一个参数，也可能是返回值？
- U-mode 执行非法特权指令后，硬件至少要留下哪些信息？
- `sepc` 不前移时，为什么可能反复触发同一个异常？
- 页内偏移为什么在虚拟地址到物理地址的转换中保持不变？
- GCC、链接器、Spike 和 GDB 分别处于程序生命周期的哪一步？
...
| 状态 | 回答的问题 | PKE 中常见例子 |
| --- | --- | --- |
| 通用寄存器 | 当前计算的数据和函数上下文在哪里 | `a0`、`sp`、`ra`、`tp` |
| 当前 PC | 下一条要执行哪条指令 | `pc`、trap 后保存的 `sepc/mepc` |
| 特权与控制状态 | 当前允许做什么、异常交给谁 | `sstatus`、`stvec`、`scause` |
| 地址转换状态 | 当前地址按哪张页表解释 | `satp`、PTE、页内偏移 |

后续每碰到一段汇编或 trap 代码，都先问：**现在在哪个特权级？PC 在哪里？寄存器由谁保存？地址按哪张页表解释？** 这四问能避免把系统调用、异常、中断和普通函数调用混在一起。
...
#### 先建立阅读目标：历史不是实验步骤，但解释了设计取舍

这一节最重要的不是年份，而是理解 RISC-V 为什么适合教学和系统研究：指令集规范开放、基础指令精简、扩展可以组合、软硬件实现能够公开讨论。PKE 使用 RISC-V，不代表操作系统原理只适用于 RISC-V；这里学习的是“操作系统怎样依赖体系结构提供的机制”。

阅读下面的历史材料时，把信息分成两类：一类是 RISC-V 的发展事实，另一类是它对实验的直接影响。前者帮助理解生态，后者包括开放工具链、Spike 模拟器、明确的特权级规范，以及可供 PKE 使用的标准 ABI。

**边界提醒：**“开源指令集”不等于“所有 RISC-V 芯片都开源”，也不等于“软件可以忽略具体实现”。开放的是 ISA 规范；处理器实现、外围设备和平台仍可能不同。
...
C 语言把“函数参数”“局部变量”“返回值”写成了抽象概念；CPU 真正执行时，只认识指令、寄存器和内存。汇编是连接二者的桥梁。后面分析 trap 入口时，如果不知道 `sp`、`ra`、`a0` 的角色，就无法判断保存现场和恢复现场是否正确。

#### 最小心智模型：一条指令做一次明确的状态变化

先只看三个元素：操作码、目的位置、源操作数。例如：

`addi a0, a0, 1` 的含义是“读取旧的 `a0`，加上立即数 1，再写回 `a0`”。它在 U-mode 中只是普通整数运算，不会因为使用了 `a0` 就触发 trap。是否能执行一条指令，取决于指令本身和当前特权级，而不是寄存器名字。

#### 从 C 函数到寄存器

RISC-V ABI 约定了寄存器的协作方式：`a0`–`a7` 主要传参数，`a0`/`a1` 也承载返回值；`ra` 保存返回地址；`sp` 指向当前栈顶；`s0/fp` 常作为栈帧指针；`tp` 常保存线程或 hart 相关身份。这里的“约定”由编译器、汇编代码和被调用函数共同遵守，不是硬件自动理解“第一个参数”。

一次普通函数调用可以按下面的顺序读：

1. 调用者把参数放入 `a0`、`a1` 等寄存器；
2. `call`/`jal` 把返回位置写入 `ra`，同时改变 PC；
3. 被调用者按需要在栈上保存 `ra`、旧 `s0` 和局部状态；
4. 函数把返回值写入 `a0`；
5. `ret` 根据 `ra` 恢复调用者的控制流。

**常见混淆：**寄存器有 ABI 角色，但寄存器里的值没有永久类型。`a0` 这一刻可以是 hart 编号，下一刻可以是 trapframe 指针，再下一刻可以是系统调用返回值。判断其含义必须结合当前执行位置。

下面保留原文的指令、寄存器、伪指令、内联汇编和函数栈材料。阅读每个例子时，始终追踪“读了谁、写了谁、PC 如何变化”。
...
如果应用可以直接修改页表、关闭中断或访问任意设备，一个错误进程就能破坏内核和其他进程。RISC-V 因此把执行权限划分为不同模式。PKE 主要涉及 M-mode、S-mode 和 U-mode：机器模式管理最底层平台能力，监管模式运行内核主体，用户模式运行应用。

#### 最小心智模型：特权级决定“这条操作是否被允许”

普通整数指令通常可在 U-mode 执行；修改关键 CSR、配置机器定时器等操作需要更高权限。CPU 在执行每条受限操作时检查当前模式。检查失败不是普通函数返回，而是产生异常，再根据委托和 trap 配置把控制权交给相应处理模式。

把 CSR 理解为“CPU 内部可编程的控制与记录字段”，不要把它当作普通内存变量。以 S-mode trap 为例：

| CSR | 在一次 trap 中的作用 |
| --- | --- |
| `stvec` | 告诉硬件 S-mode trap 入口在哪里 |
| `sepc` | 记录被打断或出错的指令位置 |
| `scause` | 记录发生了哪一类 trap |
| `stval` | 对部分异常补充出错地址或指令信息 |
| `sstatus` | 记录/控制中断使能和返回后的特权状态 |
| `sscratch` | 给 trap 入口提供一个可交换的临时上下文位置 |

#### 一次权限失败的具体流程

假设 U-mode 执行一条只允许高特权模式执行的指令：CPU 发现权限不满足，记录出错 PC 和原因，切换到目标处理模式，从该模式的 trap 向量开始执行。内核检查原因后可以终止进程、模拟操作，或做其他处理；它不能把所有异常都当作 S-mode 系统调用，因为系统调用的触发指令和语义是明确的 `ecall`。

**边界提醒：**“当前在 S-mode”与“发生了系统调用”是两个不同维度。S-mode handler 可能处理系统调用、缺页、非法指令或定时器中断；必须读 `scause`，不能只看当前模式。
...
#### 问题：CPU 怎样暂时离开当前程序，又能正确回来

操作系统必须处理三类打断顺序执行的事件：程序主动请求服务、指令执行出错、外部或定时事件到达。RISC-V 用 trap 作为统一入口机制，但统一入口不代表处理逻辑相同。

#### 用一张“trap 事件记录”分析所有情况

每次分析 trap，都填写六个字段：

| 字段 | 要问的问题 |
| --- | --- |
| 触发源 | 是 `ecall`、非法指令、访存错误，还是设备/定时器？ |
| 原模式 | 事件发生时 CPU 在 M/S/U 哪个模式？ |
| 目标模式 | 经过委托后由哪个模式接收？ |
| 硬件记录 | `xepc`、`xcause`、`xtval`、`xstatus` 得到了什么？ |
| 软件现场 | 通用寄存器保存到哪个 trapframe/栈？ |
| 返回规则 | 处理后重试原指令，还是让 `xepc` 前移？ |

#### 具体执行顺序

1. CPU 正在执行某个模式下的指令；
2. 同步异常由当前指令触发，异步中断则在满足使能条件时被接收；
3. 硬件写入原因、PC 和状态 CSR，并跳到 trap 向量；
4. 汇编入口先保存通用寄存器，建立软件可用的上下文；
5. C handler 根据 cause 分派到系统调用、缺页或中断处理；
6. handler 决定是否修改保存的 PC 和返回值；
7. 汇编出口恢复寄存器，`sret`/`mret` 恢复模式和 PC。

最容易出错的是第 6 步。`ecall` 已经完成了“请求进入内核”的动作，返回时通常要越过它；某些缺页处理完成后则要重试原访存指令。若本应越过却保持原 `sepc`，CPU 会重新执行同一条指令并形成异常循环。

**术语边界：**本文有时用“中断”泛指会打断当前执行的事件；精确分析时必须区分 exception（同步异常）与 interrupt（异步中断）。下面原文给出了更完整的分类、委托、CSR 和向量机制。
...
#### 问题：程序看到的地址为什么不必等于内存条上的位置

虚拟内存让每个进程拥有独立、连续且受保护的地址视图。CPU 发出的虚拟地址必须经过页表翻译才能访问物理内存；是否可读、可写、可执行也由页表项权限共同决定。

#### 固定使用一个地址翻译模型

以 Sv39 为例，把虚拟地址拆成多级 VPN 和页内偏移：VPN 用来逐级选择页表项，最终 PTE 给出物理页号；页内偏移不参与选页，因此原样加到物理页起始地址上。

可以把最终一步写成：

`物理地址 = PTE 给出的物理页起始地址 + 虚拟地址的页内偏移`

例如虚拟地址的页内偏移为 `0x234`，最终 PTE 指向物理页 `0x80012000`，那么最终物理地址就是 `0x80012234`。页表不会把页内第 `0x234` 个字节改成另一个偏移。

#### 查询、映射、分配不是同一个动作

- **查询页表**：沿已有 PTE 查找，缺失时应报告不存在；
- **建立映射**：写入 PTE，让 VA 指向某个已有 PA；
- **分配物理页**：从空闲物理内存取得一页；
- **处理缺页**：根据地址是否合法和访问类型，决定分配映射或拒绝；
- **切换地址空间**：修改当前 hart 使用的页表根和相关状态。

后续 Lab 2 会反复区分这些动作。特别是只做“查询并转换”的 `page_walk()` 路径，不应该悄悄创建页表，否则一次读查询就改变了进程状态。

**常见混淆：**页表属于某个地址空间。拿错页表，即使虚拟地址数值相同，也可能得到另一个进程的物理页；多 hart 场景下还要区分每个 hart 当前运行的进程和页表。
...
#### 问题：从源代码到“CPU 正在执行”经过了哪些工具

工具链不是互不相关的命令集合。它们组成一条流水线：编译器把 C 变成目标代码，汇编器处理汇编，链接器根据链接脚本组合符号和段，ELF 工具帮助检查产物，Spike 模拟 RISC-V 平台，GDB 通过符号和调试信息观察执行状态。

#### 用产物定位工具责任

| 阶段 | 主要输入/输出 | 出问题时先检查 |
| --- | --- | --- |
| 编译/汇编 | `.c`/`.S` → `.o` | 指令、ABI、编译参数 |
| 链接 | 多个 `.o` → ELF | 链接脚本、段地址、符号是否存在 |
| 装载 | ELF 段 → 内存映射 | program header、目标地址、权限 |
| 模拟执行 | ELF/镜像 → RISC-V 状态变化 | Spike 参数、平台和 trap 输出 |
| 调试 | ELF 符号 + 运行状态 | 断点、寄存器、反汇编、源代码行 |

后面看到 `objdump`、`readelf` 或 GDB 时，先问“我现在要验证的是指令、段、符号还是运行时状态”。工具的选择由问题决定，不是命令越多越可靠。

下面保留原文对 GCC、Binutils、Spike、GDB 等工具的完整说明和命令，实际使用时应同时对照第 2 章的环境验证步骤。
