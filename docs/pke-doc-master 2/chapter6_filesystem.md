...
> 文件系统最容易学乱，是因为同一个“文件”同时以路径字符串、目录项、VFS inode、RFS dinode、打开文件和数据块等形式出现。本章固定分两层理解：**VFS 层组织运行时对象和统一接口，RFS 层管理 RAM disk 上可持久化的 inode、目录项和数据块。**

#### 本章前置知识

- 知道进程通过系统调用进入内核，文件描述符只是进程表中的整数索引；
- 知道内存中的结构体修改不会自动写入磁盘/RAM disk；
- 知道一个路径由多个目录分量组成，查找必须逐级进行；
- 理解进程、页表和用户地址空间，为后面的 `exec` 与 Shell 做准备。

#### 本章学习路线

1. 先在 6.1 区分 VFS 运行时对象与 RFS 磁盘对象；
2. lab4_1 沿 `open/write/read` 调用链创建并访问普通文件；
3. lab4_2 把目录理解为“保存目录项的特殊文件”，实现迭代读取和建目录；
4. lab4_3 用两个目录项指向同一个 inode，理解硬链接计数；
5. challenge1 把路径起点从根目录扩展到进程当前工作目录；
6. challenge2 从文件系统读 ELF，并用它替换当前进程映像；
7. challenge3 组合 fork、exec、wait 和参数栈实现 Shell；
8. challengeX 把前面机制组合成可交互的小系统。

#### 一张稳定的对象关系图

`pathname → dentry → vinode → i_ops → RFS dinode → data block(s)`

其中：

| 对象 | 位于哪里 | 主要作用 |
| --- | --- | --- |
| pathname | 用户/内核字符串 | 描述查找路线，本身不保存文件 |
| dentry | VFS 内存 | 把一个名字连接到父目录和 vinode |
| vinode | VFS 内存 | 缓存文件元数据并选择 inode operations |
| struct file | 进程打开文件状态 | 保存 offset、flags 和所指对象 |
| dinode | RFS 盘上/RAM disk | 持久保存类型、大小、块号、链接数 |
| rfs_direntry | 目录文件数据 | 保存“名字 → dinode 号” |
| data block | RFS 数据区 | 保存普通文件内容或目录项数组 |

#### 学完后你应该能回答

- `vinode` 和 `dinode` 为什么不能只保留一个？
- `vinode->i_ops` 怎样让 hostfs 与 RFS 共用 VFS 接口？
- 新文件的内存结构何时才真正写入 RAM disk？
- 目录迭代中的 offset 表示字节位置还是目录项序号？
- 硬链接为什么不复制数据块，也不创建新 dinode？
- `exec` 为什么不是创建新进程，Shell 又为什么通常先 fork 再 exec？
...
#### 用一次 `open("/RAMDISK0/ramfile")` 串起两层

1. 系统调用把路径交给 VFS；
2. VFS 从根 dentry 开始逐级查找 `RAMDISK0` 和 `ramfile`；
3. 找到的 dentry 指向 vinode，vinode 描述类型、大小和具体 `i_ops`；
4. 若是 RFS 对象，inode operation 再根据 inode 号读取/修改 dinode 和数据块；
5. VFS 创建 `struct file`，把它放入当前进程文件表并返回整数 fd；
6. 后续 `read/write` 用 fd 找回 `struct file`，通过 offset 和 inode operation 完成访问。

分析任何文件调用时都写出这条链，直到明确“哪个对象在内存、哪个字段要写回盘上”。
...
- **命名：**把层次化路径解析成稳定的文件身份；
- **存储：**把变长文件内容组织到固定大小的数据块；
- **生命周期：**处理创建、打开、读写、链接、删除和回收。

目录不是纯粹的 UI 概念，它本身也是保存目录项的数据对象；文件名通常属于父目录的目录项，而不是 inode 的唯一身份。正因为名字和 inode 分开，一个 inode 才能拥有多个硬链接名。

**边界提醒：**文件大小表示有效内容长度，已分配块数表示预留了多少存储。`size = 0, blocks = 1` 可以表示文件目前没有有效字节，但已经分配一个数据块；两者不矛盾。
...
#### VFS 统一“怎么调用”，具体文件系统决定“怎么实现”

PKE 同时接触 hostfs 与 RAM disk 上的 RFS。上层 `open/read/write` 不应到处判断文件来自哪里，而是通过 vinode 的操作表分派：host 对象使用 `hostfs_i_ops`，RFS 对象使用 RFS 对应 operations。

调用方向保持为：

`进程文件系统调用 → VFS 通用检查/查找 → viop 包装 → vinode->i_ops 中的具体实现 → host 接口或 RFS 设备`

这就是 `vinode->i_ops` 的核心意义：同一 VFS 对象接口，根据挂载来源进入不同后端。

#### 共享与私有状态

目录树、dentry/vinode cache 和挂载信息通常由系统共享；文件描述符表和每次打开的 offset 属于进程/打开实例。两个 fd 指向同一 vinode 时可以看到同一文件内容，但各自 offset 是否共享要看它们是否引用同一个 `struct file`。
...
用户看到的是 fd、路径和用户缓冲区；VFS 需要先检查参数、从进程文件表解析 fd、复制或验证用户内存，再把操作转换成 vinode/dentry 和内核缓冲区上的调用。后端 RFS 不应直接信任用户指针。

按对象区分接口：

- 路径类：open、mkdir、link、unlink、chdir；
- 打开实例类：read、write、close、readdir；
- 进程映像类：exec 先通过文件接口读取 ELF，再交给装载/地址空间逻辑。

返回值也属于接口契约：fd 非负表示打开成功，read/write 返回实际字节数，目录迭代以 0 表示读到一项、`-1` 表示没有更多项。不要用打印输出替代返回值语义。
...
#### VFS 先保证路径和对象合法，再调用后端

VFS 的职责包括逐级路径查找、检查目标存在/不存在条件、验证文件类型、创建 dentry、维护 cache，以及把最终操作交给 vinode operations。比如创建文件要求父目录存在且末级名字尚不存在；硬链接要求旧文件存在、新名字不存在，并且不能把目录当普通文件链接。

因此看到 `vfs_*` 函数时，先找它做了哪些通用检查，再找最后的 `viop_*` 调用。真正修改 RFS 盘上结构的代码通常在后者选中的具体实现里。
...
- `dentry`：名字、父子目录关系、指向哪个 vinode；
- `vinode`：统一 inode 元数据、引用、superblock、`i_ops` 和文件系统私有信息；
- `file`：一次打开实例的访问模式、offset 和目标对象；
- `superblock`：一个已挂载文件系统的整体信息和后端关系。

从 RAM disk 读取的 dinode 只是内存中的临时副本；把它转换为 vinode 时，还必须初始化 `i_ops`、`sb`、`ref` 等纯运行时字段。这些字段不应原样持久化到磁盘结构中。

**判断规则：**若字段只在系统运行期间有意义（函数指针、内存引用、cache 链接），它属于 VFS 运行时对象；若重启/重新挂载后仍要恢复（类型、size、block number、nlinks），它属于盘上结构。
...
`viop_read`、`viop_write`、`viop_create`、`viop_link` 等包装函数接收 VFS 对象，再调用 `vinode->i_ops` 中对应函数。它们让 VFS 主流程不依赖 RFS/hostfs 的具体函数名。

调试调用链时不要停在 viop：继续查看当前 vinode 的 `i_ops` 在哪里初始化。若它是 `hostfs_i_ops`，操作会进入 host 文件接口；若属于 RFS，则进入 `rfs_*` 并处理 RAM disk 块。

**常见错误：**新建 vinode 时忘记设置正确 `i_ops`。此时路径查找可能成功，但第一次 read/write/create 就会调用空指针或错误后端。
...
#### dentry 树负责路径，目录文件负责持久名字

VFS 在内存中用 dentry 父子关系组织目录树；RFS 在父目录的数据块中保存 `rfs_direntry`，记录名字与 inode 号。两者表达相似关系，但生命周期不同：dentry 可被缓存或重建，盘上目录项才是重新加载后恢复名字的依据。

路径查找逐个消费分量。绝对路径从 root dentry 开始；相对路径从进程 cwd 开始；普通分量在当前目录中查目录项；`.` 保持当前节点，`..` 转向父节点（根目录需定义稳定边界）。

创建末级对象时，查找过程既要返回“没找到”，还要保留已找到的父目录和缺失名称，供 create/mkdir/link 添加新 dentry 与盘上目录项。
...
#### cache 加速身份复用，但不能改变真相来源

哈希缓存让相同路径/inode 尽量复用已有 dentry 或 vinode，避免每次都扫描盘上目录。命中 cache 后仍要遵守引用和一致性规则；删除或链接时也要同步更新 cache 与后端持久状态。

**边界提醒：**cache 中“暂时找不到”不等于盘上必然不存在，取决于缓存设计是否完整；而盘上修改成功但 cache 未更新，会让同一进程看到旧目录树。实现时必须明确先后顺序和失败回滚。
...
#### RFS 使用块号，不直接保存任意内存指针

RFS 把 RAM disk 划分为元数据区、dinode 区和数据块区。盘上结构引用数据时保存的是 RFS 内部块编号；运行时通过设备读写函数把块读到内存临时缓冲区。块号不是 host 指针，也不一定等于物理页号。

格式化负责建立初始 superblock、空闲 dinode/块状态和根目录。重新挂载时，VFS 根据这些持久结构重新构造运行时 superblock、vinode 和 dentry。

三种 dinode 类型要稳定区分：`R_FREE` 表示 inode 未使用，`R_FILE` 表示普通文件，`R_DIR` 表示目录。分配新 inode 时必须从 FREE 变为正确类型并写回盘上。
...
#### dinode 保存文件身份，direntry 保存名字

一个 dinode 由 inode number 定位，保存类型、大小、链接数和数据块编号；父目录中的 direntry 保存一个名称及其 inode number。查找文件就是先在目录数据中找到 direntry，再用 inum 读取 dinode并构造/获取 vinode。

修改内存中的 `free_dinode` 不等于创建已经持久化。只有调用类似 `rfs_write_dinode(rdev, free_dinode, free_inum)` 把结构写回 RAM disk 后，磁盘上才真正拥有新 inode 信息。父目录的新 direntry 也必须持久写入。

目录迭代 offset 表示“第几个目录项”：第一次成功调用通常从 0 读取并递增为 1，第二次从 1 读取。若目录为空，第一次就返回 `-1`，offset 不会神奇地变成 `-1`。
...
硬链接创建的是新的目录项，不是新文件内容：新名字与旧名字都指向同一个 inode number，因此通过任一名字写入都会更新同一数据。dinode 的 `nlinks` 记录当前有多少盘上目录项指向它。

unlink 先从父目录删除一个 direntry，再减少 `nlinks` 并写回。当链接数仍大于 0 时，inode 和数据块必须保留；只有满足链接和运行时引用的回收条件后才能真正释放。

**边界提醒：**VFS `ref` 统计运行时引用，RFS `nlinks` 统计持久目录链接。两者相关但不是同一个计数。
...
应用从 hostfs 读取 `/hostfile.txt`，在 RFS 的 `/RAMDISK0/ramfile` 创建文件、写入、再读回比较。目标不是实现所有文件系统功能，而是补齐 RFS 新普通文件创建，使后续 read/write 能沿正确 vinode 和数据块工作。

#### 创建调用链

`open(O_CREATE) → do_open → vfs_open/路径查找 → 末级不存在但父目录存在 → viop_create → rfs_create → 分配并初始化 dinode → 分配数据块 → 写回 dinode → 构造 vinode → 父目录添加 direntry → 返回 dentry/file/fd`

#### 初始化时逐层检查

1. 从 RAM disk 读空闲 dinode 到内存临时副本，记录 `free_inum`；
2. 把类型设为 `R_FILE`，初始 `size = 0`，按当前 RFS 设计设置 block 信息和链接数；
3. 分配 RFS 数据块，保存的是 RFS block number；
4. 调用 `rfs_write_dinode(...)` 后，盘上 inode 才真正更新；
5. 构造 vinode 时补齐 `i_ops`、`sb`、`ref`、文件系统私有信息等运行时字段；
6. 在父目录写入“ramfile → free_inum”的目录项。

`size = 0, blocks = 1` 表示还没有有效文件内容，但已预留一个数据块，后续 write 才会推进 size。不要把 blocks 误当作已写字节数。

**验证重点：**host 文件内容能写入并从 ramfile 原样读回；重新从 RFS 路径查找时仍能通过目录项和 dinode 找到文件，而不是只依赖刚创建的内存指针。
...
`opendir` 得到目录 vinode，RFS 可以把目录数据块读入 `dir_cache`；`readdir` 用 offset 选择其中一个 `rfs_direntry`，把 name 和 inum 复制到通用 `struct dir`，成功后 offset 加 1。

#### readdir 返回规则

- offset = 0：尝试读取第一个目录项；
- 成功返回 0，并把 offset 递增为 1；
- 下一次用 offset = 1 读取第二项；
- 当 offset 大于等于目录项总数，返回 `-1` 表示结束；
- 空目录总数为 0，因此第一次调用直接返回 `-1`，这是正常结束，不是读取成功。

TODO 处已经通过 offset 找到 `p_direntry`，剩余工作是复制其名字和 inode number 到输出 `dir`。不能返回指向 cache 内部短生命周期缓冲区的错误指针。

#### mkdir 的持久化链

`mkdir_u → VFS 查父目录/缺失名 → RFS 分配 R_DIR dinode和目录数据块 → 写回 dinode → 构造目录 vinode/dentry → 父目录添加 direntry`

**验证重点：**`ls` 按顺序读到每个目录项并在末尾正常停止；新目录可被再次查找，并能在其中创建和读取普通文件。
...
`link_u(oldpath, newpath)` 先解析旧 dentry/vinode，再确认新路径父目录存在且名称未占用。VFS 可为新名字创建 dentry，但它指向旧 vinode；RFS 只需在新父目录增加“new name → old inum”的 direntry，并增加旧 dinode 的 `nlinks`。

#### 三个必须一起持久化的变化

1. 旧文件链接数加 1；
2. 新父目录追加新目录项；
3. 修改后的 vinode/dinode 和目录内容写回 RAM disk。

不要分配新数据块，也不要复制文件内容。若 `/RAMDISK0/ramfile` 与 `ramfile2` 都显示 inode 1，说明它们是两个名字指向同一文件；删除第一个名字只让 `nlinks` 从 2 变 1，第二个名字仍应可读。

**验证重点：**从任一名字写入，另一名字读取到同一内容；unlink 一个名字后目录中只剩另一个，inode 和数据未被提前回收。
...
绝对路径以 `/` 开头，从 VFS root dentry 查找；相对路径从当前进程维护的 cwd 开始。`.` 表示 cwd 本身，`..` 表示父目录。因而同一个 `./file` 在不同进程或不同 cwd 下可以解析成不同 inode。

#### 逐分量算法

1. 根据路径是否以 `/` 开头选择 root 或 current->cwd；
2. 跳过重复分隔符并取下一个分量；
3. 对 `.` 保持当前 dentry，对 `..` 走父指针，对普通名字查当前目录；
4. 每走一步先确认当前对象是目录；
5. 到末级后根据 open/mkdir/link 等调用语义决定必须存在还是必须缺失。

cwd 是每进程状态，需要在进程创建/退出时正确引用和释放。根目录的 `..` 应保持在根，不能走到空指针。

**验证重点：**同一目标能通过等价绝对/相对路径访问；`chdir` 后解析起点变化；`.`、`..` 和不存在的中间目录都得到符合语义的结果。
...
应用传入可执行文件路径，如 `/bin/app_ls`。内核通过 VFS/hostfs 打开并读取 ELF，用新代码段、数据段和用户栈替换当前进程原地址空间，再把 trapframe 的入口 PC 和 SP 改为新程序初始状态。进程身份和调度关系仍属于同一个 process。

#### 完整调用链

`用户 exec(path) → syscall → VFS open/read ELF → 验证 ELF → 准备新页表映射/段 → 准备新用户栈 → 更新 mapped_info → 切换 trapframe entry/sp → 返回路径直接进入新程序`

成功的 exec 不会回到旧程序中 `exec()` 调用之后；只有失败才返回错误。实现时应尽量先验证并准备新映像，再拆除旧映像，避免中途失败让进程既无旧程序也无完整新程序。

**验证重点：**旧 `app_exec` 成功读取 `/bin/app_ls` 后，输出来自新程序；新代码/数据/栈地址有效，旧映像私有映射被正确回收或替换。
...
#### Shell 是前面机制的组合，不是特殊内核入口

给定 Shell 从 `/shellrc` 读取命令。对每条命令，父 Shell fork 子进程，子进程 exec 对应程序，父进程 wait，随后处理下一条命令。文件读取、进程复制、映像替换和同步缺一不可。

#### 一条命令的状态序列

`Shell read/parse → fork → child.a0 = 0 → child exec(command, argv) → 内核把 argv 布置到新用户栈 → child 从新 entry 运行 → child exit → parent 从 wait BLOCKED 变 READY → Shell 读取下一条`

参数传递的关键是 exec 重置用户栈后，按用户 ABI 在新栈上复制参数字符串和 `argv[]` 指针，并更新 trapframe 中的 `sp`、`a0/a1` 等初始参数寄存器。不能把内核指针直接交给新用户程序。

若不实现 wait，多个命令可能并发交错输出，Shell 也可能先退出。若 fork 后父子都 exec，Shell 本身会消失；只有子进程替换映像，父进程保留循环。

**验证重点：**`shellrc` 中命令按预期顺序执行，参数正确传入，每个子进程退出后被回收，Shell 能继续处理后续命令。
...
把前面挑战程序作为 Shell 可执行命令时，单项功能“各自能跑”还不够。它们共享进程、内存和文件系统资源，必须保证退出回收、文件 offset、cwd、COW 引用、wait 唤醒和多页文件都能组合使用。

#### 建议的集成顺序

1. 先确保 Shell 能稳定 fork/exec/wait 一个简单程序；
2. 再加入文件和目录命令，验证 pathname、fd 和多次打开；
3. 加入会创建子进程或大量内存的挑战，检查资源回收；
4. 加入大文件/多块访问，确认 inode（例如 inode 2）的 block 映射能跨越单块/单页；
5. 最后实现管道等加分功能，并明确读端、写端、阻塞与关闭语义。

多页文件不能只记录或读写第一个块。读写循环要根据当前 file offset 计算块索引和块内偏移，跨块后继续，直到完成请求或达到文件 size；dinode 的 size 与块数组必须一起更新并持久化。

**验证重点：**每加入一个功能都从全新启动执行完整 Shell 路径，确认旧命令仍能工作；不要只在已污染的内存/cache 状态下运行最后一个命令。
