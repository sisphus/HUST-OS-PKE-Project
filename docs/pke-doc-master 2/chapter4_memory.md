...
> 本章统一使用一条地址主线：**虚拟地址 VA → 多级 VPN 索引 → 页表项 PTE → 物理页起始地址 PA → 加回页内偏移**。Lab 2 的所有 TODO 都是在这条主线上做不同动作：查询、建立映射、分配物理页、处理缺页、回收，或拒绝非法访问。

#### 本章前置知识

- 知道指针值是程序使用的虚拟地址，不一定等于物理地址；
- 知道 Sv39 使用多级页表，PTE 至少包含物理页号、Valid 位和权限位；
- 知道缺页是 page fault，不等于非法指令；
- 能从 trapframe、`scause`、`stval`、`sepc` 分析一次访存异常。

#### 本章学习路线

1. 在 4.1 固定地址拆分和 PTE 心智模型；
2. lab2_1 只查询已有映射，不能创建任何页表；
3. lab2_2 学习“物理页分配—映射—解除映射—回收”的顺序；
4. lab2_3 在合法缺页时补页并重试原指令；
5. challenge1 用地址空间边界拒绝非法缺页；
6. challenge2 在页之上管理更小的堆块；
7. challenge3 把页表、进程和内存分配扩展到多 hart。

#### 五个动作绝不能混为一谈

| 动作 | 输入 | 允许改变状态吗 | 典型结果 |
| --- | --- | --- | --- |
| 页表查询 | 页表根 + VA | 否 | 找到 PTE 或返回 `NULL` |
| 地址转换 | 已有 PTE + VA offset | 否 | 得到最终 PA |
| 建立映射 | 页表根 + VA + PA + 权限 | 是 | PTE 指向物理页 |
| 分配物理页 | 物理页分配器 | 是 | 得到空闲页 PA |
| 缺页处理 | fault VA + 访问类型 + 进程区域 | 视合法性而定 | 补页并重试，或拒绝/终止 |

#### 学完后你应该能回答

- 为什么页内偏移在地址转换前后保持不变？
- 为什么 `page_walk()` 的查询模式找不到 PTE 时必须返回 `NULL`？
- 解除映射时为什么要在清 PTE 前保存物理页地址？
- 合法按需分配与非法越界访问怎样通过 `stval` 和进程区域区分？
- 两个小块共享一页时，释放一个块为什么不能解除整页映射？
- 多 hart 下为什么锁、每进程页表和每 hart 当前状态缺一不可？
...
假设最终 PTE 给出的物理页起始地址是 `0x80012000`，VA 的页内偏移是 `0x234`，则最终物理地址是 `0x80012234`。前面的多级页表只负责找到“哪一个物理页”，不会改变页内第几个字节。

后面每段代码都先问三个问题：传入的是哪张页表？当前函数只查询还是允许创建？拿到的是 PTE 地址、物理页起始地址，还是最终物理地址？这三个问题能消除大部分页表实现错误。
...
Sv39 把 VA 分成三级 VPN 和 12 位页内偏移。`satp` 提供根页表物理页号；每一级 VPN 选择一个 PTE。非叶子 PTE 指向下一级页表，叶子 PTE 才给出最终物理页和 R/W/X 等权限。

页表遍历函数通常同时支持两种策略：只查找，或在中间页表缺失时创建。调用者必须显式选择。地址转换、权限检查等只读操作使用查询策略；建立新映射时才允许为缺失层级分配页表页。

**边界提醒：**PTE 中的物理页号需要移位还原为物理页起始地址；最终 PA 还要加 VA 的页内偏移。不能把 PTE 指针本身当作 PA。
...
内核镜像、设备区域和可分配 RAM 位于同一物理地址空间，但不是所有物理地址都能交给用户。初始化物理页分配器时必须先划清：哪些页已经被内核/平台占用，哪些页可以进入空闲集合。

分配器返回一个物理页，只表示获得了该页的所有权；用户程序还不能通过某个 VA 访问它。只有把该 PA 写入正确进程页表的叶子 PTE，并设置合适权限后，映射才建立。

回收顺序反过来：先确认没有仍在使用的映射/小块，再解除 PTE，最后把物理页归还分配器。跳过所有权检查会产生 use-after-free 或两个用途同时使用一页。
...
内核和每个用户进程都通过页表看到自己的逻辑地址空间。用户代码、用户栈、trapframe 和 trap 入口页各有预定区域及权限；它们可能映射到不同物理页。有些内核区域采用一一映射，但这只是特定区域的设计，不代表所有 VA 都等于 PA。

`current` 指向当前进程状态，而进程结构应持有属于自己的页表和映射记录。切换进程时，CPU 使用的页表也要切换。多 hart 同时运行不同进程时，更不能靠一份全局“当前页表”表达所有 hart 的状态。

阅读下面布局图时，为每个区间标记四项：虚拟范围、物理来源、U/S 可见性和 R/W/X 权限。地址落在范围内只是第一步，访问类型还必须满足权限。
...
- `page_walk()` 一类遍历函数：定位 VA 对应的 PTE；是否创建中间层必须由参数决定；
- map 一类函数：需要时建立页表层级并写叶子 PTE；
- unmap 一类函数：读取旧 PTE、清除映射，是否回收物理页由上层所有权决定；
- VA-to-PA 查询：调用只读 walk，提取 PTE 中的 PA，再加 offset；
- 页分配器：只管理物理页，不自动知道某个进程 VA。

**关键不变量：**调用者传入哪张页表，就只在那张页表中查找或修改。不能偷偷使用全局页表，否则同一 VA 可能被翻译成另一个进程的物理页。
...
给定应用已经拥有若干用户映射，本 Lab 只要求查询一个 VA 的最终 PA。它不是“为地址补映射”的 Lab，因此查询过程中不能分配中间页表或物理页。

#### 端到端数据流

`传入的 pagetable + VA → page_walk(create = false) → PTE → 检查 Valid/叶子 → 取物理页起始地址 → 加 VA 页内偏移 → PA`

如果任一级不存在、PTE 无效或不是可转换的叶子，应返回 `NULL`/失败值。最重要的是使用调用者传入的那张页表；若改用其他用户或全局页表，同一 VA 会得到错误映射。

#### 实现推理

1. 用“不创建”模式查 PTE；
2. 找不到时直接返回 `NULL`，不改变页表；
3. 从 PTE 中提取物理页起始地址 `pa`；
4. 用 VA 的低 12 位作为页内偏移；
5. 返回 `pa + offset`。

**验证重点：**用跨页内不同偏移的地址检查结果，确认不是只返回物理页起始地址；同时确认查询不存在的地址不会让页表或空闲页数量变化。
...
`空闲物理页 → alloc_page() 取得 PA → 在当前进程页表建立 VA→PA 映射 → 用户访问 → 从 PTE 取回旧 PA → 清除 PTE/刷新转换状态 → free_page(PA)`

分配和映射是两个动作：只分配 PA，用户 VA 仍不可访问；只写映射但没有合法拥有 PA，则可能映射到别人正在使用的内存。

#### 为什么回收顺序不能反

解除映射需要知道该 VA 原先指向哪个物理页。因此必须先从仍然有效的 PTE 中提取旧 PA，再清除 PTE；若先把 PTE 置零，物理页号就丢失了，之后无法正确回收。

清 PTE 也不等于物理页已经释放。只有确认没有其他映射或子分配仍使用该页后，才能把 PA 交还物理页分配器。

**验证重点：**观察映射前访问不可用、映射后读写正确、解除后同一 VA 不再有效，并确认物理页能够被后续分配复用。
...
应用执行的是合法 store 指令，但目标页尚未映射，因此 CPU 产生 `CAUSE_STORE_PAGE_FAULT`。`stval` 保存导致 fault 的 VA，例如 `0x7fffd000`；`sepc` 指向需要重试的那条 store。

#### 端到端调用链

`U-mode store → 页表翻译失败 → store page fault → S-mode trap → 读取 stval → 检查地址合法 → 分配物理页 → 建立用户可写映射 → 保持 sepc → sret → 原 store 重试成功`

这里与系统调用相反：handler 修复的是这条访存指令的前置条件，所以 `sepc` 应保持不变。若把它前移，应用的 store 会被跳过，程序继续运行却没有真正写入数据。

#### 实现时逐项检查

1. cause 必须是相应 load/store/instruction page fault，而不是其他异常；
2. fault VA 来自 `stval`，映射时按页对齐；
3. 为当前进程分配页，并给出与访问类型相容的权限；
4. 记录映射所有权，便于进程结束时回收；
5. 不修改 `sepc`，让硬件重新执行原访存。
...
按需分配区域中的合法地址和完全越界的坏指针，都可能产生 page fault。handler 必须结合 `stval`、访问类型和进程地址空间范围判断，而不能见到 fault 就分配一页。

#### 判定顺序

1. 读取 fault cause，区分取指、读和写；
2. 读取 `stval` 得到 fault VA；
3. 检查 VA 是否位于允许按需增长/分配的用户区域；
4. 检查所需权限是否与区域规则一致；
5. 合法则补页并重试；非法则返回失败或终止当前进程。

非法访问不能通过简单前移 `sepc`“解决”。那只会跳过一条本应执行的用户指令并破坏语义；保持 `sepc` 又不处理则会反复 fault。正确做法是明确拒绝该进程的无效访问。

**验证重点：**合法区域首次访问能补页，重复访问不再 fault；越界地址不会获得新映射，也不会陷入无限异常循环。
...
应用的 `malloc` 请求可能远小于一页。实现可以先取得一个物理页并映射到用户堆，再把这一页切成多个块，分别记录空闲/占用状态。于是有两层生命周期：块的生命周期和承载它们的页的生命周期。

假设 `p` 和 `q` 是同一页 `pa` 中的两个块：

`0x400000 ──页表映射──→ pa`，而 `p`、`q` 只是 `pa` 内不同偏移。

释放 `q` 只把对应块标记为空闲；只要 `p` 仍在使用，整页映射必须保持不变。若此时调用 `user_vm_unmap()` 或 `free_page()`，`p` 也会立即失效。

#### 什么时候才能回收整页

只有当该页上的所有块都为空闲，并且分配器元数据确认没有活动引用时，才可以解除 VA→PA 映射并回收物理页。块状态、页映射和物理页所有权必须按这个顺序更新。

**验证重点：**分配两个同页块，释放其中一个后另一个仍可读写；全部释放后页面才可整体回收并再次利用。
...
两个 hart 可以使用数值相同的 VA，因为它们运行的进程拥有不同页表。页表应放进 `process` 结构体，hart1 运行进程时使用该进程的 pagetable，而不是 hart0 的页表或一份全局页表。

每 hart 运行状态也要分开，例如 `g_itrframe[hartid]`、`argv[hartid]`、当前进程和用户/内核栈。共享的物理页分配器则需要并发保护。

#### 为什么“先读再写”不是锁

若锁实现只是读取 0 再写 1，两个 hart 可能同时读到 0，并都认为自己获得锁。获取锁必须使用原子的读改写操作；锁保护的临界区要覆盖空闲页元数据的完整检查和更新，而不是只保护最后一条赋值。

#### 启动与退出边界

共享内核内存初始化如 `kern_vm_init()` 的全局部分只能完成一次，并在 boot barrier 后供其他 hart 使用；每个进程/每 hart 的页表和运行映射则分别建立。退出时不能让 hart0 一结束就 shutdown，否则 hart1 可能仍在读写内存；所有 hart 完成后再由约定 hart 全局关闭。

**验证重点：**两个 hart 的相同 VA 能落到各自映射，物理页分配无重复所有权，trapframe/应用参数互不覆盖，并且早结束的 hart 不会关闭仍在运行的系统。
